    ...
    match plan {
        ...
        LogicalPlan::TableScan(table_scan) => {
            ...
            let (new_projection, new_projection_deep) = if projection.is_some() {
                ...
                indices.into_mapped_indices_deep(
                    |idx| {
                        projection_clone[idx]
                    },
                    |idx| {
                        match &projection_deep_clone {
                            None => {vec![]}
                            Some(m) => {
                                match m.get(&projection_clone[idx]) {
                                    None => {vec![]}
                                    Some(v) => {
                                        v.clone()
                                    }
...
fn outer_columns(expr: &Expr, columns: &mut HashMap<Column, Vec<String>>) {
...
fn outer_columns_helper_multi<'a>(
    ...
    columns: &mut HashMap<Column, Vec<String>>,
) {
...
mod tests {
    ...
    use datafusion_expr::{binary_expr, build_join_schema, builder::table_scan_with_filters, col, count, expr::{self, Cast}, lit, logical_plan::{builder::LogicalPlanBuilder, table_scan}, max, min, not, try_cast, when, AggregateFunction, BinaryExpr, Expr, Extension, Like, LogicalPlan, Operator, Projection, UserDefinedLogicalNodeCore, WindowFrame, WindowFunctionDefinition, Literal};
