...
use datafusion_common::{Column, DFSchema, DFSchemaRef, DataFusionError, Result, ScalarValue, TableReference};
use datafusion_expr::{build_join_schema, Expr, Join, LogicalPlan, Projection, Subquery, SubqueryAlias, TableScan, Union};
...
use datafusion_common::TableReference::Bare;
use datafusion_expr::expr::{Alias, ScalarFunction};
use datafusion_functions::unicode::translate;
        ...
        if options.optimizer.deep_column_pruning_flags & FLAG_ENABLE != 0 {
            ...
            if new_plan_transformed.transformed {
                ...
                let new_plan = new_plan.transform_up(|p| {
                    ...
                    match p {
                        LogicalPlan::Window(_) => Ok(Transformed::yes(p.recompute_schema()?)),
...
pub fn deep_plan_transformer(
    ...
    plan.transform_down(|plan| {
        ...
        if options.optimizer.deep_column_pruning_flags & FLAG_ENABLE_SUBQUERY_TRANSLATION != 0 {
            ...
            if matches!(plan, LogicalPlan::TableScan(_)) {
                ...
                for expr in plan.expressions().iter() {
                    match expr {
                        Expr::Alias(Alias { expr: alias_source, relation, name }) => {
        ...
        match plan {
                    ...
                    if are_exprs_plain(&expr) && cols_level.len() > 0 {
                        ...
                        'col_iterate: for (my_col, my_deep_projections) in my_pdc.columns.iter() {
                            ...
                            for my_deep_projection in my_deep_projections.iter() {
                                ...
                                if my_deep_projection == "*" {
                                    ...
                                    let mut new_pdc = PlanWithDeepColumnMap {
                        ...
                        if my_new_modified_columns.len() > 0 {
                            ...
                            // - change the check so that it verifies that the schemas can be casted / rewritten, instead of being equal
                            {
                                ...
                                let proj_functional_dependencies = schema.functional_dependencies();
                                ...
                                for (idx, (mut c, f)) in schema.iter().enumerate() {
                                    let newc = c.cloned();
            ...
            LogicalPlan::EmptyRelation(_) => return return Ok(Transformed::new(plan, false, TreeNodeRecursion::Jump)),
...
pub fn reproject_for_deep_schema(
    ...
    if projection.is_some() && projection_deep.is_some() {
        ...
        let new_fields = new_inner_projected_schema.fields().iter()
            .map(|fi| {
...
pub fn convert_plans_with_deep_column_to_columns_map(input: &Vec<PlanWithDeepColumnMap>) -> Vec<DeepColumnColumnMap> {
    input.iter().map(|pdc|  pdc.columns.clone()).collect()
}

pub fn filter_expressions_for_table(input: &Vec<DeepColumnColumnMap>, table_name: String) -> Vec<DeepColumnColumnMap> {
...
pub fn compact_list_of_column_maps_to_single_map(input: &Vec<DeepColumnColumnMap>) -> DeepColumnColumnMap {
...
pub fn transform_column_deep_projection_map_to_usize_index_type_and_clean_up_stars(input: &DeepColumnColumnMap, schema: &DFSchemaRef) -> DeepColumnIndexMap {
...
pub fn transform_column_deep_projection_map_to_usize_index_type_and_clean_up_stars_for_plain_schema(input: &DeepColumnColumnMap, schema: &SchemaRef) -> DeepColumnIndexMap {
    let mut output: DeepColumnIndexMap = HashMap::new();
    for (col, values) in input.iter() {
        // This is needed because of the way DF checks qualified columns
        let name = col.name();
...
pub fn fix_deep_projection_according_to_table_schema(input: &DeepColumnColumnMap, schema: &DFSchemaRef) -> DeepColumnColumnMap {
    ...
    for (col, deep_projection_specifiers) in input {
        ...
        if let Some(col_idx_in_schema) = schema.maybe_index_of_column(&col_to_check) {
            ...
            for projection_specifier in deep_projection_specifiers {
                // ADR: FIXME
                let fixed_projection_specifier_pieces = fix_possible_field_accesses(&schema, col_idx_in_schema, projection_specifier).unwrap();
                let fixed_projection_specifier = fixed_projection_specifier_pieces.join(".");
...
pub fn merge_value_in_column_map(map: &mut DeepColumnColumnMap, col: &Column, new_value: String) {
...
pub fn find_all_projections_for_column(needle: &Column, haystack: &Vec<PlanWithDeepColumnMap>, start: usize, end: usize) -> Vec<String> {
    let mut end = end;
    if start == end {
        end += 1;
    }
...
pub fn fix_possible_field_accesses(schema: &DFSchemaRef, field_idx: usize, deep_projection_specifier: &String) -> Result<Vec<String>> {
    // info!("fix_possible_field_accesses {} {}", field_idx, deep_projection_specifier);
    if deep_projection_specifier == "*" {
        return Ok(vec!["*".to_string()]);
    }
    let rest = deep_projection_specifier.split(".").map(|x| x.to_string()).collect::<Vec<String>>();
...
pub fn are_exprs_plain(exprs: &Vec<Expr>) -> bool {
    for pexpr in exprs.iter() {
        match pexpr {
            Expr::Alias(Alias { expr:alias_inner_expr, .. }) => {
