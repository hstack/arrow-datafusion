...
use substrait::proto::{rel::RelType, AggregateRel, ConsistentPartitionWindowRel, CrossRel, DdlRel, ExchangeRel, ExpandRel, ExtensionMultiRel, ExtensionSingleRel, FetchRel, FilterRel, HashJoinRel, JoinRel, MergeJoinRel, NestedLoopJoinRel, ProjectRel, Rel, SetRel, SortRel, WriteRel};
...
fn inputs(rel: &Rel) -> Vec<&Rel> {
    match &rel.rel_type {
        Some(rel_type) => match rel_type {
            ...
            RelType::Ddl(ddl_rel) => ddl_rel.view_definition.as_deref().into_iter().collect(),
            ...
            RelType::Window(window_rel) => window_rel.input.as_deref().into_iter().collect(),
            RelType::Expand(expand_rel) => expand_rel.input.as_deref().into_iter().collect(),
    ...
    fn map_children<F: FnMut(Self) -> Result<Transformed<Self>>>(
        ...
        if let Some(rel_type) = self.rel_type {
            let t = match rel_type {
                ...
                RelType::Fetch(p) => {
                    let FetchRel {
                        common, input, advanced_extension, offset_mode, count_mode
                    } = *p;
                    transform_option_box(input, &mut f)?.update_data(|input| {
                ...
                RelType::Aggregate(p) => {
                    let AggregateRel {
                        common, input, groupings, measures, grouping_expressions, advanced_extension
                    } = *p;
                    transform_option_box(input, &mut f)?.update_data(|input| {
                ...
                RelType::Set(p) => {
                    ...
                    let new_inputs: Vec<_> = inputs
                        .into_iter()
                        .map(|input| {
                            let transformed = transform_box(Box::new(input), &mut f).unwrap();
                ...
                RelType::ExtensionMulti(p) => {
                    ...
                    let new_inputs: Vec<Rel> = inputs
                        ...
                        .collect();
                    if transformed_any {
                        Transformed::yes(RelType::ExtensionMulti(
                            ExtensionMultiRel {
                                common,
                                inputs: new_inputs,
                                detail,
                        ...
                        Transformed::no(RelType::ExtensionMulti(
                            ExtensionMultiRel {
                                common,
                                inputs: new_inputs,
                                detail,
                ...
                RelType::Join(p) => {
                    let JoinRel {
                        ...
                        left,
                        right,
                        expression, post_join_filter, r#type, advanced_extension
                    } = *p;
                    let mut transformed_any = false;
                    ...
                    if transformed_any {
                        Transformed::yes(RelType::Join(Box::new(
                            JoinRel {
                                common,
                                left: new_left.data,
                        ...
                        Transformed::no(RelType::Join(Box::new(
                            JoinRel {
                                common,
                                left: new_left.data,
                                right: new_right.data,
                ...
                RelType::Cross(p) => {
                    ...
                    if transformed_any {
                        Transformed::yes(RelType::Cross(Box::new(
                            CrossRel {
                                common,
                                left: new_left.data,
                                right: new_right.data,
                        ...
                        Transformed::no(RelType::Cross(Box::new(
                            CrossRel {
                                common,
                                left: new_left.data,
                                right: new_right.data,
                ...
                RelType::Write(p) => {
                    let WriteRel {
                        table_schema, op, input, create_mode, output, common, write_type
                    } = *p;
                    transform_option_box(input, &mut f)?.update_data(|input| {
                ...
                RelType::Ddl(p) => {
                    let DdlRel {
                        table_schema, table_defaults, object, op, view_definition, common, write_type
                    } = *p;
                ...
                RelType::HashJoin(p) => {
                    let HashJoinRel {
                        common, left, right, left_keys, right_keys, keys, post_join_filter, r#type, advanced_extension
                    } = *p;
                    let mut transformed_any = false;
                    let new_left = transform_option_box(left, &mut f)?;
                    if new_left.transformed {
                    ...
                    if transformed_any {
                        Transformed::yes(RelType::HashJoin(Box::new(
                            HashJoinRel {
                                common,
                                left: new_left.data,
                                right: new_right.data,
                        ...
                        Transformed::no(RelType::HashJoin(Box::new(
                            HashJoinRel {
                                common,
                                left: new_left.data,
                                right: new_right.data,
                ...
                RelType::MergeJoin(p) => {
                    ...
                    if transformed_any {
                        Transformed::yes(RelType::MergeJoin(Box::new(
                            MergeJoinRel {
                                common,
                                left: new_left.data,
                                right: new_right.data,
                        ...
                        Transformed::no(RelType::MergeJoin(Box::new(
                            MergeJoinRel {
                                common,
                                left: new_left.data,
                                right: new_right.data,
                ...
                RelType::NestedLoopJoin(p) => {
                    let NestedLoopJoinRel {
                        common, left, right, expression, r#type, advanced_extension
                    } = *p;
                    let mut transformed_any = false;
                    let new_left = transform_option_box(left, &mut f)?;
                    if new_left.transformed {
                ...
                RelType::Window(p) => {
                    let ConsistentPartitionWindowRel {
                        common, input, window_functions, partition_expressions, sorts, advanced_extension
                    } = *p;
                    transform_option_box(input, &mut f)?.update_data(|input| {
                ...
                RelType::Exchange(p) => {
                    let ExchangeRel {
                        common, input, partition_count, targets, advanced_extension, exchange_kind
                    } = *p;
                    transform_option_box(input, &mut f)?.update_data(|input| {
